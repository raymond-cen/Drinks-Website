...
function SavedRecipeDetailed ({detailedRecipe}) {
  ...
  const handleRemove = async () => {
    try {
      await apiCall(`recipe/save/${detailedRecipe.recipe.recipeid}`, "DELETE");
      ...
      <Box sx={{display: "flex", marginTop: "10px"}}>
        <Box>
          ...
          <Box sx={{
            ...
            <Button onClick={handleRemove} fx={{backgroundColor: "#c8957c", color: "#4b0f20"}}>Remove from Saved</Button>
          </Box>
        </Box>
...
export default SavedRecipeDetailed